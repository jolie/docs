...
To invoke a web service's operation, we need to create a compatible Jolie interface. To do that we exploit the tool [wsdl2jolie](/docs/web_services/wsdl2jolie), which is part of the Jolie trunk installation.
...
To expose the port `MyServiceSOAPPort` as a web service, we exploit the tool [jolie2wsdl](/docs/web_services/jolie2wsdl) that generates the corresponding WSDL document to be attached to the file:
