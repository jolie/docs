...
An outputPort defines a target endpoint connected with a service and it allows to exchaneg messages with it.
...
### The behaviour
The behaviour contains the code of the three operations where ach of them exploits at least one operation of the `CalculatorService`. Operation `factorial` uses `mul@Calculator`, operation `average` uses `sum@Calculator` and `div@Calculator`, finally operation `percentage` uses `div@Calculator` and `mul@Calculator`. 
...
**Tips:** character `#`, when used before a variable path, plays the role of operatior _size_ and it returns the number of the elements of the related vector. In the example, we read the statement `#request.term` as _the number of elements of vector term within the node request_.
