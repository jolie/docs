...
```
The former permits to identify the session thanks to _auth_token_ whereas the latter exploits the _session_id_. Both of them identifies the same session, but the token _auth_token_ is used for identifying the messages related to the _identity_provider_ whereas the _session_id_ it is used for identifying the session initiated by the user into the application. Onde logged, the _auth_token_ is not more useful, whereas the _session_id_ can be used by the user for accessing the application. 
